({
	init : function(component, event, helper) {
		...
		executeQuery.setParams({ query: "SELECT Id, CodigoClienteAS400__c  FROM Account WHERE Id='" + recordId +"'" });
		...
		.then( $A.getCallback( function( res ) {
			...
			if( res && res[0] && res[0].CodigoClienteAS400__c  ) {
				app_ctc.instance.as400 = res[0].CodigoClienteAS400__c 
				app_ctc.instance.renderUrl = true
				codigoAs400= res[0].CodigoClienteAS400__c 
                ...
                .then( $A.getCallback( function( res ) {
                    if( res && res[0] && res[0].URL__c ) {
                        app_ctc.instance.url =  res[0].URL__c+'&Tipo=1&Valor='+codigoAs400;
                    }
